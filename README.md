...
```bash
git clone https://github.com/j-laiti/wellby-android.git
cd wellby-android
```
...
4. Add it to the `app/` directory of your Android Studio project
...
```bash
cp PrivateKeysTemplate.kt PrivateKeys.kt
```

Open `PrivateKeys.kt` and add your API keys:

```kotlin
object PrivateKeys {
    const val OPENAI_API_KEY = "your-openai-api-key"
    const val YOUTUBE_API_KEY = "your-youtube-api-key"
    const val FIREBASE_PPG_FUNCTION_URL = "your-firebase-function-url"
    const val FIREBASE_FLAGGED_MESSAGE_FUNCTION_URL = "your-firebase-function-url"
    const val OPENAI_ASSISTANT_ID = "your-openai-assistant-id"
}
```

> **Note:** `PrivateKeys.kt` is gitignored and will not be committed to the repository.
...
You can modify the letter mappings in `AuthManager.kt` under `checkStudyCode()` to suit your study design.
...
- `BluetoothController.kt` - Handles BLE connection and data streaming
- `HRVDataManager.kt` - Manages HRV data storage and processing
...
Coach assignment is automatic based on the first letter of the student's name (to distribute load), but this logic can be modified in `AuthManager.kt`.
...
- **Study Codes** - Modify the code suffix meanings in `AuthManager.kt` to match your cohorts
...
- **Bluetooth Permissions** - Android requires Location permissions to be granted for BLE functionality. Ensure you explain this to participants in your consent process
